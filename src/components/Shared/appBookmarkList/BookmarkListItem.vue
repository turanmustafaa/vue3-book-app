<template>
     <div class="bg-white flex flex-col gap-x-3 rounded-md shadow-sm">
            <div class="p-3">
              <a href="#" class="hover:text-black font-bold text-l mb-1 text-gray-600 text-center">{{ item.head }}</a>
              ...
              <div class="text-xs text-gray-400 mt-2 flex justify-between">
                <a href="#" class="hover:text-black"> {{item.user.username}} </a>
                <span>14 Mart</span>
              </div>
            </div>
            <div class="bg-red-200 p-1 text-red-900 text-center text-sm">{{item.category.category}}</div>
          </div>
</template>
<script>
...
export default {
  ...
  computed : {
    ...mapGetters(["_getCurrentUser","_getCurrentUserId","_userLikes","_userBookmarks"]),
